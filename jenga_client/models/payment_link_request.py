...
class PaymentLinkRequest(BaseModel):
    ...
    payment_link: PaymentLinkRequestPaymentLink = Field(alias="paymentLink")
    notifications: Optional[List[StrictStr]] = Field(default=None, description="Modes for customer to receive payment link")
    __properties: ClassVar[List[str]] = ["customers", "paymentLink", "notifications"]

    @field_validator('notifications')
    def notifications_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        for i in value:
            if i not in set(['EMAIL', 'SMS']):
                raise ValueError("each list item must be one of ('EMAIL', 'SMS')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.payment_link:
            _dict['paymentLink'] = self.payment_link.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "paymentLink": PaymentLinkRequestPaymentLink.from_dict(obj["paymentLink"]) if obj.get("paymentLink") is not None else None,
